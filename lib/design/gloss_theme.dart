...
class GlossSurfaces {
  ...
  /// Restrained 3D gloss plate — soft vertical gradient + depth around the whole
  /// surface (not bottom-heavy only). Soft ambient wrap + balanced drop + very
  /// subtle top highlight keep it professional, not synthetic.
  static BoxDecoration get plate => BoxDecoration(
        ...
        boxShadow: [
          ...
          // Soft ambient wrap — depth reads on sides/top, not only base
          BoxShadow(
          ...
          // Very subtle top highlight (restraint — no strong rim)
          BoxShadow(
  ...
  /// Navy gloss plate — same restrained full-depth model as [plate].
  /// Used for selected chips (contrast).
  ...
  /// Field outer shell — same restrained cyan gloss language, pill radius.
  static BoxDecoration get fieldPlate => BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: GlossColors.navy.withValues(alpha: 0.10),
            blurRadius: 10,
