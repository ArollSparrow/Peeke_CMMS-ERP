...
class _OrgTeamScreenState extends ConsumerState<OrgTeamScreen> {
  ...
  Future<void> _bulkInvite() async {
      ...
      builder: (ctx) {
        return StatefulBuilder(
          builder: (ctx, setLocal) {
            return AlertDialog(
              title: Text('Bulk invite', style: GlossSurfaces.logoMark),
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text(
                      'One work email per line. Same role for all.',
                    ...
                    GlossSurfaces.fieldShell(
                      child: TextField(
                        ...
                        decoration: GlossSurfaces.compactField('Emails')
                            .copyWith(hintText: 'a@company.com\nb@company.com'),
    ...
    final lines = ctrl.text
        ...
        .toList();
    if (lines.isEmpty) {
      setState(() => _error = 'Add at least one email');
      return;
    }
    var okCount = 0;
    var failCount = 0;
    ...
    try {
      for (final email in lines) {
        ...
        try {
          ...
          final data = res.data;
          if (data is Map && data['error'] != null && data['status'] == null) {
            failCount++;
          } else {
            okCount++;
          }
      ...
      setState(() {
        _message =
            'Bulk invite: $okCount sent${failCount > 0 ? ', $failCount failed' : ''}';
      });
      if (okCount > 0) {
        await logOrgActivity(
          ref,
          action: 'bulk_invite',
          summary: 'Bulk invite: $okCount sent as ${OrgRoles.label(role)}',
          metadata: {'ok': okCount, 'failed': failCount, 'role': role},
        );
      }
    } finally {
      if (mounted) setState(() => _busy = false);
      ...
      body: ListView(
        ...
        children: [
          ...
          if (canInvite) ...[
            ...
            Center(
              child: TextButton(
                onPressed: _busy ? null : _bulkInvite,
                child: Text('Bulk invite…', style: GlossSurfaces.logoMark),
              ),
            ),
            if (_error != null) ...[
              const SizedBox(height: GlossSurfaces.fieldGap),
