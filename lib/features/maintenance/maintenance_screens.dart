          ...
          return ListView(
            ...
            children: [
              ...
              TextButton(
                onPressed: () async {
                  final ok = await showDialog<bool>(
                    ...
                    builder: (ctx) => AlertDialog(
                      ...
                      content: Text('"${p.title}" will be removed. Existing WOs are not affected.'),
                      actions: [
                        TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
                ...
                child: const Text('Delete plan', style: TextStyle(color: GlossColors.danger)),
              ),
            ],
          );
        },
        ...
        child: async.when(
          ...
          data: (items) {
            if (items.isEmpty) {
              return ListView(children: [
                const SizedBox(height: 48),
                const Center(child: Text('No maintenance jobs yet')),
                Center(
                  child: FilledButton(
                    onPressed: () => context.push('/maintenance/jobs/new'),
                    ...
                    subtitle: Text(
                      [
                        ...
                        if (r.performedAt != null)
                          r.performedAt!.toLocal().toString().split('.').first,
                      ].join(' · '),
