...
class WorkHubScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          _tile(context, icon: Icons.assignment_add, title: 'New work request',
              subtitle: 'Raise WR against a system', route: '/work/requests/new'),
          _tile(context, icon: Icons.inbox_outlined, title: 'Work requests',
              subtitle: pending == null ? 'Review & convert' : '$pending pending',
              route: '/work/requests'),
          _tile(context, icon: Icons.handyman_outlined, title: 'Work orders',
              subtitle: open == null ? 'Execute jobs' : '$open open',
              route: '/work/orders'),
          _tile(context, icon: Icons.add_task_outlined, title: 'New work order',
              subtitle: 'Create WO directly (skip WR)', route: '/work/orders/new'),
        ],
      ),
    );
  }
  ...
  Widget _tile(BuildContext context,
      {required IconData icon, required String title, required String subtitle, required String route}) {
    return Card(
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: () => context.push('/work/requests/new'), icon: const Icon(Icons.add)),
        ],
      ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: () => context.push('/work/orders/new'), icon: const Icon(Icons.add)),
        ],
      ),
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          data: (items) {
            if (items.isEmpty) {
              return ListView(children: const [
                SizedBox(height: 48),
                Center(child: Text('No work orders yet')),
                SizedBox(height: 8),
                Center(child: Text(
