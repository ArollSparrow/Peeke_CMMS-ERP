...
- When mail cannot deliver, the app returns a shareable **action_link** (WhatsApp / SMS).
...
Edge Function invite-org-member (service role)
  ...
  • inviteUserByEmail (preferred) OR generateLink → action_link
        │
        ▼
...
1. **Never open the shareable link while signed in as the owner** (or any other account). That caused the “Kiai became owner” incident: password/name updated on the wrong Auth user.
...
5. On mail failure / rate limit → return `action_link` for WhatsApp/SMS.
